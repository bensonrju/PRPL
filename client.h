 ...
 * Description: Retuns the size of a file pointer's file
 * 
 * Parameters:
...
unsigned int pack_packets(
    ...
    *pkt_ptr = (packet_s *) malloc(packet_size * num_packets);
    ...
    for(pkt_idx = 0; pkt_idx < (num_packets - 1); pkt_idx++) {
        ...
        // Set the incoming flag of the payload to default to 1
        //  indicating there are more packets to follow
        pkt_arr[pkt_idx].incoming = 1;
    ...
    // Set the last packet's incoming flag to 0
    //  indicating there are no more packets to follow
    pkt_arr[pkt_idx].incoming = 0;
